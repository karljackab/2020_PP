...
int main(int argc, char *argv[])
{
    ...
    for(cur_thread_id=1; cur_thread_id<thread_num; cur_thread_id++)
        pthread_create(thread+cur_thread_id, NULL, child_toss, (void*)&sub_toss_num);
    ull remain_toss_num = toss_num-sub_toss_num*(thread_num-1);
    pthread_create(thread, NULL, child_toss, (void*)&remain_toss_num);
